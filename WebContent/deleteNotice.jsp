...
sql = "select title,content,date,adminname,ntcid from "+tableName+" order by date desc";
//创建语句对象之后，通过语句对象进行查询操作，返回结果集合
rs=stmt.executeQuery(sql);
if(!rs.first())//数据库中没有这条记录
{
	out.println("对不起，目前公告为空");
		  ...
		  <tbody>
			<tr id="jive-message-780144" class="<%=classType%>" valign="top">
			  <td class="jive-first" width="1%"><!-- 个人信息的table -->
						...
						<table border="0" cellpadding="0" cellspacing="0" width="100%">
						  <tbody>
							<tr valign="top">
							  ...
							  <td style="padding: 0px;" width="99%"><%=rs.getString("adminname") %>
				...
				<a href="deleteNoticeFl.jsp?ntcid=<%=id[i]%>"> 删除该公告</a>
</span>
					  </td>
					</tr>
				  </tbody>
			  ...
			  <td class="jive-last" width="99%"><table border="0" cellpadding="0" cellspacing="0" width="100%">
				<tbody>
				  <tr valign="top">
					...
					<td width="97%">主题：<%=rs.getString("title")%></a>
